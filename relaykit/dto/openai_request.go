...
func (r *OpenAIResponsesRequest) IsRemoteCompactionV2() bool {
	...
	if err := kitutil.Unmarshal(r.Input, &input); err != nil || len(input) == 0 {
		return false
	}
	...
	if err := kitutil.Unmarshal(finalInput["type"], &finalInputType); err != nil || finalInputType != "compaction_trigger" {
		return false
	}

	var clientMetadata map[string]json.RawMessage
	if err := kitutil.Unmarshal(r.ClientMetadata, &clientMetadata); err != nil {
		return false
	}
	...
	if err := kitutil.Unmarshal(turnMetadataValue, &turnMetadataJSON); err != nil {
		return false
	}
	...
	if err := kitutil.UnmarshalJsonStr(turnMetadataJSON, &turnMetadata); err != nil || turnMetadata.Compaction == nil {
		return false
	}
