...
func ChatCompletionsResponseToResponsesResponse(resp *dto.OpenAITextResponse, id string, metadata ...*convmeta.ResponsesToolRestoreMetadata) (*dto.OpenAIResponsesResponse, *dto.Usage, error) {
	if resp == nil {
	...
	var restore *convmeta.ResponsesToolRestoreMetadata
	if len(metadata) > 0 {
		restore = metadata[0]
	}
...
func chatToolCallToResponsesOutput(toolCall dto.ToolCallRequest, responseID string, index int, status string, metadata *convmeta.ResponsesToolRestoreMetadata) (dto.ResponsesOutput, error) {
