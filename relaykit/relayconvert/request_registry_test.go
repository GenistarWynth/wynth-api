...
func TestConvertResponsesRequestCarriesToolRestoreMetadata(t *testing.T) {
	...
	result, err := ConvertRequest(nil, &convmeta.Values{}, types.RelayFormatOpenAI, request)

	require.NoError(t, err)
	require.NotNil(t, result.ResponsesToolRestore)
	assert.Equal(t, convmeta.ResponsesNamespacedTool{Namespace: "mcp", Name: "lookup"}, result.ResponsesToolRestore.ReverseToolNames["mcp__lookup"])
	assert.True(t, result.ResponsesToolRestore.ToolSearchDeclared)
}
