...
func convertOAIChatResponseToOAIResponses(_ context.Context, info convmeta.Meta, response any) (any, *dto.Usage, error) {
	...
	return oaichat.ChatCompletionsResponseToResponsesResponse(chatResponse, id, convmeta.OptionsOf(info).ResponsesToolRestore)
}
...
func newOAIChatToOAIResponsesStreamState(options ResponseStreamOptions) any {
	...
	state := oaichat.NewChatToResponsesStreamState(id, strings.TrimSpace(options.Model), options.ResponsesToolRestore)
