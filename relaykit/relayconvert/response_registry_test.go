...
func TestResponseRegistryCarriesResponsesToolRestoreMetadata(t *testing.T) {
	restore := &convmeta.ResponsesToolRestoreMetadata{
		ReverseToolNames: map[string]convmeta.ResponsesNamespacedTool{
	...
	chat.Choices[0].Message.SetToolCalls([]dto.ToolCallRequest{{
		...
		Function: dto.FunctionRequest{Name: "mcp__lookup", Arguments: `{"q":"x"}`},
	}})

	result, err := ConvertResponse(nil, &convmeta.Values{Options: &convmeta.Options{ResponsesToolRestore: restore}}, types.RelayFormatOpenAIResponses, chat)
	...
	assert.Equal(t, "mcp", response.Output[1].Namespace)

	state, err := NewResponseStreamState(types.RelayFormatOpenAI, types.RelayFormatOpenAIResponses, ResponseStreamOptions{
		ID: "resp_1", Model: "gpt-test", ResponsesToolRestore: restore,
	})
	require.NoError(t, err)
	toolIndex := 0
	results, err := ConvertStreamResponseChunk(nil, nil, state, &dto.ChatCompletionsStreamResponse{Choices: []dto.ChatCompletionsStreamResponseChoice{{
	...
	require.NoError(t, err)
	require.NotEmpty(t, results)
	event, ok := results[0].Value.(ChatToResponsesStreamEvent)
	require.True(t, ok)
	require.NotNil(t, event.Payload.Item)
	assert.Equal(t, "lookup", event.Payload.Item.Name)
	assert.Equal(t, "mcp", event.Payload.Item.Namespace)
}
