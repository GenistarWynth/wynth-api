...
type RequestResult struct {
	...
	Steps                []RequestStep
	ResponsesToolRestore *convmeta.ResponsesToolRestoreMetadata
}

type requestConversionValue struct {
	value                any
	responsesToolRestore *convmeta.ResponsesToolRestoreMetadata
}
...
func executeRequestSteps(c context.Context, info convmeta.Meta, from types.RelayFormat, target types.RelayFormat, request any, converter string, quality RequestConverterQuality, specs []RequestConverterSpec) (*RequestResult, error) {
	...
	var responsesToolRestore *convmeta.ResponsesToolRestoreMetadata
	for _, spec := range specs {
		...
		if converted, ok := current.(*requestConversionValue); ok {
			current = converted.value
			if converted.responsesToolRestore != nil {
				responsesToolRestore = converted.responsesToolRestore
			}
		}
	...
	return &RequestResult{
		...
		Quality:              quality,
		Steps:                steps,
		ResponsesToolRestore: responsesToolRestore,
	}, nil
}
...
func convertResponsesRequestToChat(_ context.Context, _ convmeta.Meta, request any) (any, error) {
	...
	conversion, err := oairesponses.ConvertResponsesRequestToChat(responsesRequest)
	if err != nil {
		return nil, err
	}
	return &requestConversionValue{
		value: conversion.Request,
		responsesToolRestore: &convmeta.ResponsesToolRestoreMetadata{
			ReverseToolNames:   conversion.ReverseToolNames,
			ToolSearchDeclared: conversion.ToolSearchDeclared,
		},
	}, nil
}
