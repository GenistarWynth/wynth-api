...
export function isAdvancedCustomIncomingPathAllowed(
  ...
  return isConverterPathAllowed(incomingPath.trim(), converter)
}
...
export function getAdvancedCustomStats(value: string | undefined): {
  ...
  for (const route of routes) {
    ...
    const label = getAdvancedCustomIncomingPathLabel(incomingPath)
    if (seenRouteTypeLabels.has(label)) continue
