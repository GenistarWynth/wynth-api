...
export function UpstreamRatioSyncTable({
    ...
    return upstreamNames.reduce<Record<string, UpstreamBulkSelectState>>(
      (states, upstreamName) => {
        ...
        for (const row of filteredData) {
          for (const ratioType of getAlignedRatioTypes(
            row.ratioTypes,
            [upstreamName],
            ratioTypeFilter
          )) {
            ...
            if (isSelectableUpstreamValue(upstreamValue)) {
              selections.push({
                model: row.model,
                ratioType,
                value: upstreamValue as number | string,
  ...
  const handleBulkSelect = useCallback(
    (upstream: string) => {
      onSelectValues(bulkSelectStateByUpstream[upstream]?.selections ?? [])
    },
