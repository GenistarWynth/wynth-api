...
interface UserTokenEditDrawerProps {
  open: boolean
...
export function UserTokenEditDrawer({
  open,
  onOpenChange,
  token,
  userId,
  onSuccess,
}: UserTokenEditDrawerProps) {
  const { t } = useTranslation()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [advancedOpen, setAdvancedOpen] = useState(false)

  // Fetch models for this admin session (the current admin's models list)
  ...
  // Fetch groups for this admin session
  ...
  // Prefill form with the current token values whenever the drawer opens
  useEffect(() => {
    if (open && token) {
      form.reset(transformApiKeyToFormDefaults(token))
    }
    if (!open) {
  ...
  const onSubmit = async (data: ApiKeyFormValues) => {
    ...
    try {
      // Build the full object — backend PUT overwrites all fields, so we must
      // send the complete payload (no partial patch).
      const payload = transformFormDataToPayload(data)
      const result = await updateUserToken(userId, {
        ...payload,
        id: token.id,
      })
      if (result.success) {
