...
export function TimingMetricsCell(props: TimingMetricsCellProps) {
  ...
  const firstTokenTimeMs = props.frtMs
  const hasFirstTokenTime = firstTokenTimeMs != null && firstTokenTimeMs > 0
  const firstTokenLabel = hasFirstTokenTime
    ? formatUseTime(firstTokenTimeMs / 1000)
    : t('N/A')
        ...
        {props.isStream && (
          <div className='flex items-baseline gap-1.5'>
            ...
            <span
              className={cn(
                'tabular-nums',
                hasFirstTokenTime ? 'text-success' : 'text-muted-foreground'
              )}
            >
              {firstTokenLabel}
            </span>
              ...
              <TooltipTrigger
                render={
                  <CircleAlert
                    ...
                    aria-label={`${t('Stream Status')}: ${t('Error')}`}
                  />
                }
              />
              <TooltipContent>
