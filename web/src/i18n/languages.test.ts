...
describe('interface language normalization', () => {
  test('exposes every supported interface language', () => {
    assert.deepEqual(
      INTERFACE_LANGUAGE_OPTIONS.map((language) => language.code),
      ['zh', 'en', 'fr', 'ru', 'ja', 'vi', 'zh-TW']
    )
  })

  test('preserves distinct simplified and traditional Chinese resources', () => {
    assert.equal(convertDetectedLanguage('zh-CN'), 'zh')
    assert.equal(convertDetectedLanguage('zh_TW'), 'zh-TW')
    assert.equal(convertDetectedLanguage('zh-Hant-TW'), 'zh-TW')
    assert.equal(normalizeInterfaceLanguage('zhCN'), 'zh')
    assert.equal(normalizeInterfaceLanguage('zhTW'), 'zh-TW')
  })

  test('returns valid Intl locales and safely falls back for malformed values', () => {
    assert.equal(toIntlLocale('zh'), 'zh-CN')
    assert.equal(toIntlLocale('zh-TW'), 'zh-TW')
