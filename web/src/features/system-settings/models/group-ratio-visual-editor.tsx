...
function groupPricingSignature(rows: GroupPricingRow[]): string {
  ...
  return JSON.stringify({
    ...
    topupGroupRatio: parseGroupRatioMap(serialized.TopupGroupRatio),
  })
}
...
function sourceGroupPricingSignature(
  ...
  return JSON.stringify({
    ...
    topupGroupRatio: parseGroupRatioMap(topupGroupRatio),
  })
}
...
export const GroupRatioVisualEditor = memo(function GroupRatioVisualEditor({
  ...
  const registry = useMemo<RegistryEntry[]>(() => {
    ...
    return [...names].map((name) => ({
      name,
      ratio: serializeGroupRatioDraft(normalizeGroupRatioDraft(ratioMap[name])),
    }))
...
function GroupDetailSheet(props: GroupDetailSheetProps) {
  ...
  const detail = useMemo(() => {
    ...
    const topupMap = parseGroupRatioMap(props.topupGroupRatio)
    const usableMap = parseGroupDescriptionMap(props.userUsableGroups)
    const overrideMap = parseNestedGroupRatioMap(props.groupGroupRatio)
    const specialRules = parseSpecialUsableRules(props.groupSpecialUsableGroup)
    ...
    for (const rule of specialRules) {
      if (rule.targetGroup !== name) continue
