...
function formatAutoPriorityNumber(value: number | undefined, digits?: number) {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    return '-'
  }
  ...
  const tooltip = snapshot
    ? t(
        'Auto priority may overwrite manual edits. Last run: {{time}}. Nominal rate: {{rate}}x; price score: {{price}}. Cache factor: {{cacheFactor}}x; cache score: {{cache}}; source: {{cacheSource}}; prior factor: {{cachePrior}}x; own confidence: {{cacheConfidence}}. Availability score: {{availability}}. First token score: {{firstToken}}. Throughput score: {{throughput}}. Effective cost diagnostic: {{cost}}x.',
        {
          ...
          cachePrior: formatAutoPriorityNumber(snapshot.cache_factor_prior, 4),
          ...
          cost: formatAutoPriorityNumber(snapshot.effective_cost_multiplier, 3),
 ...
 * notation; the precise value stays available in the tooltip.
 */
...
export function BalanceCell({ channel }: { channel: Channel }) {
  ...
  let remainingBadgeVariant: StatusVariant = variant
  if (channel.type === 57) {
