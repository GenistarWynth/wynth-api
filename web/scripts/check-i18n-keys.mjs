    ...
    if (ent.isDirectory()) {
      ...
      while ((m = T_CALL.exec(s))) {
        ...
        if (!keys.has(key)) {
          ...
          missing.get(key).add(path.relative(root, p).replaceAll('\\', '/'))
        }
      }
    }
  }
...
if (missing.size === 0) {
  console.log(
    `i18n:check OK — all t() literal keys present in en.json (${keys.size} keys).`
  )
  process.exit(0)
}
console.error(
  `i18n:check FAILED — ${missing.size} t() key(s) missing from en.json:\n`
)
...
for (const f of [...byFile.keys()].sort()) {
  console.error(`  ${f}`)
  for (const k of byFile.get(f).sort()) {
