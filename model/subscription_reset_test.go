...
func TestAdminResetUserSubscriptionsByPlanUsesCurrentDatabasePlanWithStaleCache(t *testing.T) {
	truncateTables(t)
	...
	plan := &SubscriptionPlan{
		Id:                      9701,
		Title:                   "Current custom reset plan",
		PriceAmount:             10,
	...
	stalePlan := *plan
	stalePlan.QuotaResetPeriod = SubscriptionResetNever
	stalePlan.QuotaResetCustomSeconds = 0
	require.NoError(t, cache.SetWithTTL(
		subscriptionPlanCacheKey(plan.Id),
		stalePlan,
		subscriptionPlanCacheTTL(),
	))

	result, err := AdminResetUserSubscriptionsByPlan(501, plan.Id, true)
	...
	assert.Equal(t, sub.LastResetTime+3600, sub.NextResetTime)
}

func TestAdminResetPlanSubscriptionsDoesNotPopulateSharedPlanCache(t *testing.T) {
	truncateTables(t)
	...
	_, found, err := cache.Get(subscriptionPlanCacheKey(plan.Id))
	require.NoError(t, err)
	...
	_, found, err = cache.Get(subscriptionPlanCacheKey(plan.Id))
	require.NoError(t, err)
	assert.False(t, found, "quota-reset transactions must not populate the shared plan cache")
}
