...
// NewSSRFProtectionFromFetchSetting builds an SSRFProtection from persisted fetch settings.
...
func parseIPLiteralForPolicy(host string) (net.IP, bool) {
...
// ValidateNetworkTarget validates a host and port before a network connection.
func (p *SSRFProtection) ValidateNetworkTarget(host string, port int) error {
	...
	if ip, ok := parseIPLiteralForPolicy(host); ok {
...
// ValidateResolvedIP validates a domain result immediately before connecting to it.
...
func (p *SSRFProtection) ValidateURL(urlStr string) error {
	...
	// Literal IPs and domains without result filtering need no DNS validation here.
	if _, ok := parseIPLiteralForPolicy(host); ok || !p.ApplyIPFilterForDomain {
		return nil
	}
