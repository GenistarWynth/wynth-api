...
func runDueDeadChannelRecoveryBatch(testUserID int) {
	channels, err := model.GetAllChannels(0, 0, true, false)
	if err != nil {
		common.SysError("dead channel recovery list failed: " + err.Error())
		return
	}
	...
	for _, channel := range candidates {
		// Reuse the monitor probe path so success can auto-enable via the same
		// ShouldEnableChannel gate, and so results are recorded for observability.
		runChannelMonitorProbe(channel, testUserID)
		// After a failed probe, bump status_time so the next random delay is
		// measured from this attempt rather than the original death time.
		if refreshed, err := model.GetChannelById(channel.Id, true); err == nil && refreshed != nil {
			if refreshed.Status == common.ChannelStatusAutoDisabled {
				info := refreshed.GetOtherInfo()
				info["status_time"] = common.GetTimestamp()
				info["status_reason"] = "dead channel recovery probe failed; rescheduled"
				refreshed.SetOtherInfo(info)
				_ = refreshed.SaveWithoutKey()
			}
		}
...
func AutomaticallyTestChannels() {
	...
	autoTestChannelsOnce.Do(func() {
		...
		for range ticker.C {
			...
			// Post-mortem recovery for auto-disabled channels without monitor.
			testUserID, err := resolveChannelTestUserID(nil)
			if err != nil {
				common.SysError("dead channel recovery skipped: " + err.Error())
				continue
			}
			runDueDeadChannelRecoveryBatch(testUserID)
		}
	})
}
