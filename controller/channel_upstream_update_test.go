...
func TestFetchAdvancedCustomModelsAppliesHeaderOverrideAfterRouteAuth(t *testing.T) {
	...
	defer server.Close()
	allowChannelUpstreamUpdateTestServer(t, server.URL)
...
func TestFetchAdvancedCustomModelsRejectsUnlistedPort(t *testing.T) {
...
func withChannelUpstreamUpdateFetchSetting(t *testing.T, allowPrivate bool, allowedPorts ...string) {
	t.Helper()
	...
	fetchSetting.AllowedPorts = append([]string(nil), allowedPorts...)
...
func allowChannelUpstreamUpdateTestServer(t *testing.T, serverURL string) {
	t.Helper()

	parsedURL, err := url.Parse(serverURL)
	require.NoError(t, err)
	require.NotEmpty(t, parsedURL.Port())
	withChannelUpstreamUpdateFetchSetting(t, true, parsedURL.Port())
}
